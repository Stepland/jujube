...
if not os.path.isdir(args.dest):
	print(f"Error: destination {args.dest} is not a directory")
	exit(1)

# find dependencies
dependencies = ldd(args.files)
print(f"Found {len(dependencies)} new dependencies")
...
for file in dependencies:
	...
	if not os.path.isfile(dest_file):
		...
		copy_count += 1
print(f"Copied {copy_count} dependencies to {args.dest}")
