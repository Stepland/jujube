...
namespace Input {
    ...
    KeyMapping::KeyMapping(std::unordered_map<Button, Key> button_to_key) : m_button_to_key(button_to_key) {
    ...
    KeyMapping::KeyMapping(std::unordered_map<Key, Button> key_to_button) : m_key_to_button(key_to_button) {
    ...
    void KeyMapping::set_button_to_key(const Button& button, const Key& key) {
    ...
    std::optional<Button> KeyMapping::key_to_button(const Key& key) {
        if (m_key_to_button.find(key) == m_key_to_button.end()) {
            return {};
        } else {
    ...
    std::optional<Key> KeyMapping::button_to_key(const Button& button) {
    ...
    void from_json(const nlohmann::json& j, KeyMapping& km) {
        std::unordered_map<Button, Key> map;
